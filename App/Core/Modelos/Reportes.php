...
class Modelos_Reportes extends Sfphp_Modelo 
{
	...
	{
		$query = "SELECT vta.fecha, usr.nombre, SUM(det.subtotal) venta, MAX(gto.monto) gasto, SUM(pro.costo*det.cantidad) costo, SUM(det.subtotal) - MAX(gto.monto) - SUM(pro.costo*det.cantidad) utilidad
		FROM ventas vta
