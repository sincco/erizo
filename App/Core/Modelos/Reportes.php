...
class Modelos_Reportes extends Sfphp_Modelo 
{
	...
	public function utilidades($desde, $hasta)
	{
		$query = "SELECT vta.fecha, usr.nombre, 
			...
			FORMAT(SUM(det.subtotal) - IFNULL(MAX(gto.monto),0) - SUM(pro.costo*det.cantidad),3) * (con.porcentajeSocio1/100) socio1,
			FORMAT(SUM(det.subtotal) - IFNULL(MAX(gto.monto),0) - SUM(pro.costo*det.cantidad),3) * (con.porcentajeSocio2/100) socio2,
			FORMAT(SUM(det.subtotal) - IFNULL(MAX(gto.monto),0) - SUM(pro.costo*det.cantidad),3) * (con.porcentajeSocio3/100) socio3,
			FORMAT(SUM(det.subtotal) - IFNULL(MAX(gto.monto),0) - SUM(pro.costo*det.cantidad),3) * (con.porcentajeSocio4/100) socio4
		FROM ventas vta
	...
	public function ventas($desde, $hasta) {
		$query = "SELECT nombre, FORMAT(SUM(efectivo),3) efectivo, FORMAT(SUM(credito),3) credito, FORMAT(SUM(tarjeta),3) tarjeta, FORMAT(SUM(efectivo)+SUM(credito)+SUM(tarjeta), 3) venta
			...
			GROUP BY nombre";
		$_detalle = $this->db->query($query);
		$query = "SELECT 'TOTAL' nombre, FORMAT(SUM(efectivo),3) efectivo, FORMAT(SUM(credito),3) credito, FORMAT(SUM(tarjeta),3) tarjeta, FORMAT(SUM(efectivo)+SUM(credito)+SUM(tarjeta), 3) venta
			FROM (
				SELECT nombre,
					case when trim(tipo) = 'Efectivo' then monto else 0 end efectivo,
					case when trim(tipo) = 'Crédito' then monto else 0 end credito,
		...
		array_push($_detalle, array_pop($this->db->query($query)));
		return $_detalle;
	}
}